defmodule Day8 do
  ...
  def part1(file, wide, tall) do
    ...
    zeros =
      dat
      |> Enum.map(fn layer -> layer |> Enum.count(fn c -> c == "0" end) end)

    working = Enum.at(dat, Enum.find_index(zeros, fn cnt -> cnt == zeros |> Enum.min() end))
    ones = working |> Enum.count(fn c -> c == "1" end)
    twos = working |> Enum.count(fn c -> c == "2" end)
    ones * twos
  end
  ...
  def part2(file, wide, tall) do
    ...
    dat =
      Enum.reduce(dat, canvas, fn layer, acc ->
