defmodule Day5Test do
  ...
  test "Day5.part1" do
    assert Day5.part1() |> Intcode.state_io_out == 5_577_461
  end

  test "Day5.part2" do
    assert Day5.part2() |> Intcode.state_io_out == 7_161_591
  end

  test "1101,100,-1,4,0" do
    assert "1101,100,-1,4,0" |> Intcode.prog() |> Intcode.state_mem ==  "1101,100,-1,4,99"
  end

  test "1002,4,3,4,33" do
    assert "1002,4,3,4,33" |> Intcode.prog() |> Intcode.state_mem ==  "1002,4,3,4,99"
  end

  test "day2 sample1" do
    assert "1,0,0,0,99" |> Intcode.prog() |> Intcode.state_mem == "2,0,0,0,99"
  end

  test "day2 sample2" do
    assert "2,3,0,3,99" |> Intcode.prog() |> Intcode.state_mem == "2,3,0,6,99"
  end

  test "day2 sample3" do
    assert "2,4,4,5,99,0" |> Intcode.prog() |> Intcode.state_mem == "2,4,4,5,99,9801"
  end

  test "day2 sample4" do
    assert "1,1,1,4,99,5,6,0,99" |> Intcode.prog() |> Intcode.state_mem  == "30,1,1,4,2,5,6,0,99"
  end
  ...
  test "je/jne" do
    assert "3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9" |> Intcode.prog(0) |> Intcode.state_io_out == 0
    assert "3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9" |> Intcode.prog(1) |> Intcode.state_io_out == 1
  end
